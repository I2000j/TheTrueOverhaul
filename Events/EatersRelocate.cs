...
namespace TrueOverhaul.Events
{
    public class EatersRelocate
    {
		...
		public static void StartEatersRelocate()
		{
			//Set to no invasion
			...
			if (Main.invasionType == 0)
			{
				...
				if (num > 0)
				{
					//Invasion setup
					Main.invasionType = -1;
					TrueOverhaulWorld.EatersRelocateUp = true;
		...
		public static void EatersRelocateWarning()
		{
			String text = "";
			if (Main.invasionX == (double)Main.spawnTileX)
			{
				text = Language.GetTextValue("Mods.TrueOverhaul.EatersRelocateStart");
			}
			if(Main.invasionSize <= 0)
			{
				text = Language.GetTextValue("Mods.TrueOverhaul.EatersRelocateEnd");
			}
			if (Main.netMode == 0)
			{
		...
		public static void UpdateEatersRelocate()
		{
			if(TrueOverhaulWorld.EatersRelocateUp)
			{
				...
				if(Main.invasionSize <= 0)
				{
					TrueOverhaulWorld.EatersRelocateUp = false;
					EatersRelocateWarning();
					Main.invasionType = 0;
		...
		public static void CheckEatersRelocateProgress()
		{
